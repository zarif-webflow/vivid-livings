...
const setupCombobox = (comboboxInput, initialResults = []) => {
  ...
  const resultContainer = comboboxInput.closest(SELECTORS.comboboxWrapper)?.querySelector(SELECTORS.resultContainer);
  if (!resultContainer) {
    console.log("Result container wasnt found for", comboboxInput);
    return;
  }
  const resultList = resultContainer.querySelector(SELECTORS.resultList);
  if (!resultList) {
    console.log("Result list wasnt found for", comboboxInput);
    return;
  }
  const resultItem = resultList.querySelector(SELECTORS.resultItem);
  if (!resultItem) {
  ...
  const setModalPosition = () => {
    computePosition(comboboxInput, resultContainer, {
      ...
      setStyle(resultContainer, {
        left: `${x}px`,
        top: `${y}px`,
        transform: placement.includes("bottom") ? `translateY(${comboboxMargin})` : `translateY(calc(-1 * ${comboboxMargin}))`
      });
    });
  };
  ...
  const selectResultItem = (index) => {
    ...
    const textElement = selectedItem.querySelector(SELECTORS.resultItemValue);
    if (!textElement) {
  ...
  const renderResults = (results) => {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      const textElement = listItem.querySelector(SELECTORS.resultItemValue);
      if (!textElement) {
...
export { SELECTORS as S, setupCombobox as s };
